...
links to them — worth reading before touching a file a finished package owned. One
package stays at the top level despite its code being complete, because a **human** still
has to do something: HARD-12 (the CSP is report-only until someone flips it). Moving it
into `done/` would hide the only part still outstanding. HARD-02b and HARD-07 were the
...
| [HARD-23](HARD-23-invoice-automation-auth.md) Make.com cannot authenticate to the invoice endpoint | `routes/index.js` (`/league/sendInvoices`), `contactusController.js` | B | — |
...
| [HARD-23](HARD-23-invoice-automation-auth.md) | not started | | **The annual invoices did not go out on 1 Sep 2026.** `POST /league/sendInvoices` came from `Make/production` with no session and got a **302 to /login**; SEC-3 had gated it and nothing checked who was already calling it. Recovered that evening with a superadmin browser `fetch` — 18 clubs, all successful, which is the right number. **The inverse of the same day's `/fixture/rearrangement` lesson**: that one started with a grep for callers and was safe; this one skipped it. Gating an endpoint is a change to its callers. Silent because three things line up and all still do — Make.com treats 302 as success, the controller answers **200** to a refusal (`["not the right date for invoices"]`), and the date check means any failure lasts a **year**. Fix reuses `requireAuditCaller` from HARD-07. Also: there is **no UI trigger at all**, so devtools was the only recovery path. |

...
links to them — worth reading before touching a file a finished package owned. Two
packages stay at the top level despite their code being complete, because a **human** still
has to do something: HARD-07 (set `AUDIT_EMAIL_TO` or nothing is ever sent) and HARD-12
(the CSP is report-only until someone flips it). Moving those into `done/` would hide the
only part still outstanding. HARD-02b was a third until 1 Sep 2026, when its bucket steps
were run; its one remaining *optional* extra became HARD-22 rather than keeping a finished
package open.

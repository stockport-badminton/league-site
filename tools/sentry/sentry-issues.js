#!/usr/bin/env node
/**
 * Sentry issue triage helper (read-only) for the league site.
 *
 * Reads issues from Sentry's REST API so we can pick off low-hanging fruit.
 * The app's Sentry is a browser-only integration (see views/header.ejs), so the
 * issues are client-side JS errors.
 *
 * Setup (once):

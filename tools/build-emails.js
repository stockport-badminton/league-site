 ...
 * mjml is a devDependency. Compiling here and committing the result means production
 * renders a plain EJS template with the ejs it already has — the runtime path is exactly
 * what it was before MJML existed. It also makes the diff reviewable: you can see in git
 * what the email will actually look like.
 *
 * Compiling in the image instead would make mjml a production dependency for the sake of
 * a build step, which is the wrong trade for templates that change a few times a year.
 *
 * Ported from the Tameside site (1055eea0), which hit all three of the traps below
 * first.
 *

...
exports.init = async function () {
  try {
    const [rows] = await (await db.otherConnect()).query(
      'SELECT id, name FROM division ORDER BY rank'
    );
  ...
  try {
    ...
    // Only seasons we can actually serve. season.getAll() already restricts to
    // those with an archived team<season> snapshot - offering a season without
    // one would 500 the page it links to.
    const rows = await seasonModel.getAll();
    // The current season is the empty-value option in the select rather than an
    // entry here, so picking it produces a clean URL with no season segment.
    // Its own label comes from the season table so it reads the same way.
    const [currentRows] = await (await db.otherConnect()).query(
      'SELECT label FROM season WHERE name = ?', [current]
    );
    _currentSeasonLabel = (currentRows[0] && currentRows[0].label) || seasonLabel(current);
    _seasons = rows
      .filter(function (r) { return r.name !== current; })
      .map(function (r) {
        return { value: r.name, label: r.label || seasonLabel(r.name) };
      });
  } catch (err) {
